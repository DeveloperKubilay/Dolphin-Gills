...
socket = io("ws://" + settings.serverurl, {
    auth: {
...
async function createserver(id, os, ports, disks, ram, cpu, ngrok, tryl) {
    try {
        ...
        await disks.map(async (x) => {
            ...
            if (x.type === "download") {
                ...
                await axios({
                    url: x.url, signal: controller.signal, responseType: "stream", onDownloadProgress: (progressEvent) => {
                        if (x.main) {
                    ...
                    controller.abort()
                    if (!controllered) {
                        if (tryl < settings.overdownloadtry) {
                            controllered = true; setTimeout(() => createserver(id, os, ports, disks, ram, cpu, ngrok, tryl + 1), 5000)
    ...
    let stoped = 0, ngrok = ""; if (pids.filter(z => z.machine === id).length) {
        setInterval(function () {
            try {
                ...
                var storage = []
                fs.readdirSync("./servers/" + id).map((c) => {
                    if (c === "start.txt") return;
        ...
        async function downloadnewdisk(id, x, tryl) {
            ...
            await axios({ url: x.url, signal: controller.signal, responseType: "stream" }).then((response) => {
                ...
                .catch(() => {
                    ...
                    if (!controllered) {
                        if (tryl < settings.overdownloadtry) {
                            controllered = true; setTimeout(() => downloadnewdisk(id, x, tryl + 1), 5000)
                        } else { controllered = true; socket.emit("servers", { machine: id, err: true, code: "The disc could not be downloaded" }) }
                    }
                })
        }
...
setInterval(async () => {
    totaldisksize = 0;
    await fs.readdirSync("./servers").map(async (y) => {
        await fs.readdirSync("./servers/" + y).map(async (x) => {
            totaldisksize = totaldisksize + fs.statSync("./servers/" + y + "/" + x).size
        })
    })
    if (totaldisksize) {
        socket.emit("servers", { totaldisksize: Math.round((totaldisksize / 1024) / 1024) })
